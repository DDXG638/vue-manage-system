    ...
    <div>
        <div class="crumbs">
            <el-breadcrumb separator="/">
                ...
                <el-breadcrumb-item>导入新闻</el-breadcrumb-item>
            </el-breadcrumb>
        </div>
        <div class="handle-box">
            <el-select v-model="addNewsClassId" placeholder="请选择新闻分类">
                <el-option v-for="(item, index) in tagsLists" :key="item.classid" :label="item.classname" :value="item.classpath"></el-option>
            </el-select>
            <el-button type="primary" icon="el-icon-refresh" @click="importData">开始导入</el-button>
        </div>
        ...
        methods: {
            ...
            async submit(){
                ...
                if (this.picArr.length > 2) {
                    this.formData.titlepic = this.picArr[0];
                    this.formData.titlepic2 = this.picArr[1];
                    this.formData.titlepic3 = this.picArr[2];
                } else {
                    this.formData.ptitlepic = this.picArr[0];
                }
                let res = await fetch('post', 'saveNewsDetail', this.formData);
            ...
            // 导入第三方数据
            async importData() {
                let tagId = this.addNewsClassId;
                if (tagId.length < 1) {
                    this.$message.error("请选择新闻分类！");
                    return;
                }
                this.fullscreenLoading = true;
                let res = await fetch("get", "importNewsData", {tag: tagId});
                this.fullscreenLoading = false;
                if (res.code === 0) {
                    this.$alert('成功导入' + res.data.successInsert.length + '条数据', '导入数据', {
